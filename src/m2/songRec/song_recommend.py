...
def gen_bfs_score(graph, seeds, max_depth=3):
    ...
    for seed in seeds:
        ...
        while queue:
            ...
            visited.add(current)
            if depth > 0:
                score_map[current] += 1.0 / depth  # closer nodes score higher
...
def recommend(graph_path, feature_path, seed_ids, top_k=10, w_sim=0.5, w_hot=0.3, w_bfs=0.2):
    ...
    df = df.select("artist_name", "song_id", "title", *feature_cols).dropna()
    ...
    # Normalize similarity and hotttnesss
    ...
    print("\nTop Recommendations:")
    for song_id, title, artist_name, score in top_recommendations:
        print(f"{title} (ID: {song_id}) by {artist_name} -> Score: {score:.4f}")
