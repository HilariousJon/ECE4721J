...
ARRAY_DESC_ARTIST_TERMS_FREQ = (
    "array of term (Echo Nest tags) frequencies for an artist"
)
...
def fill_hdf5_from_song(h5, song):
    ...
    metadata.cols.artist_location[0] = (
        song.get_artist_location().location.encode("utf-8")
        if song.get_artist_location().location
        else ""
    )
    metadata.cols.artist_longitude[0] = song.get_artist_location().longitude
    metadata.cols.artist_name[0] = (
        song.artist_name.encode("utf-8") if song.artist_name else ""
    )
...
def fill_hdf5_from_track(h5, track):
    ...
    idsplitter_7digital = lambda x: (
        int(x.split(":")[2]) if x and x.split(":")[0] == "7digital" else -1
    )
    ...
    group.segments_loudness_max_time.append(
        np.array(map(lambda x: x["loudness_max_time"], track.segments), dtype="float64")
    )
        ...
        for songidx in xrange(nSongs):
            ...
            if not summaryfile:
                if counter == 0:  # we're first row
                    ...
                    row["idx_similar_artists"] = h5.root.metadata.similar_artists.shape[
                        0
                    ]
            ...
            if not summaryfile:
                if counter == 0:  # we're first row
                    ...
                    row["idx_segments_confidence"] = (
                        h5.root.analysis.segments_confidence.shape[0]
                    )
                    row["idx_segments_pitches"] = (
                        h5.root.analysis.segments_pitches.shape[0]
                    )
                    row["idx_segments_timbre"] = h5.root.analysis.segments_timbre.shape[
                        0
                    ]
                    row["idx_segments_loudness_max"] = (
                        h5.root.analysis.segments_loudness_max.shape[0]
                    )
                    row["idx_segments_loudness_max_time"] = (
                        h5.root.analysis.segments_loudness_max_time.shape[0]
                    )
                    row["idx_segments_loudness_start"] = (
                        h5.root.analysis.segments_loudness_start.shape[0]
                    )
                    row["idx_sections_start"] = h5.root.analysis.sections_start.shape[0]
                    row["idx_sections_confidence"] = (
                        h5.root.analysis.sections_confidence.shape[0]
                    )
                    row["idx_beats_start"] = h5.root.analysis.beats_start.shape[0]
                    row["idx_beats_confidence"] = (
                        h5.root.analysis.beats_confidence.shape[0]
                    )
                    row["idx_bars_start"] = h5.root.analysis.bars_start.shape[0]
                    row["idx_bars_confidence"] = h5.root.analysis.bars_confidence.shape[
                        0
                    ]
                    row["idx_tatums_start"] = h5.root.analysis.tatums_start.shape[0]
                    row["idx_tatums_confidence"] = (
                        h5.root.analysis.tatums_confidence.shape[0]
                    )
            row.append()
            ...
            if not summaryfile:
                if counter == 0:  # we're first row
                    ...
                    row["idx_artist_mbtags"] = h5.root.musicbrainz.artist_mbtags.shape[
                        0
                    ]
            row.append()
...
def create_song_file(h5filename, title="H5 Song File", force=False, complevel=1):
    ...
    table = h5.createTable(
        group, "songs", DESC.SongMetaData, "table of metadata for one song"
    )
    r = table.row
    ...
    table = h5.createTable(
        group, "songs", DESC.SongAnalysis, "table of Echo Nest analysis for one song"
    )
    r = table.row
    r.append()  # filled with default values 0 or '' (depending on type)
    table.flush()
    ...
    table = h5.createTable(
        group, "songs", DESC.SongMusicBrainz, "table of data coming from MusicBrainz"
    )
    r = table.row
    r.append()  # filled with default values 0 or '' (depending on type)
    table.flush()
